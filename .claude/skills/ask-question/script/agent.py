...
class RAGKnowledgeAgent(AgentBase):
    ...
    def _ensure_connection(self):
        """确保 Milvus 连接正常，如果需要则重新创建客户端，并确保 collection 已加载"""
        try:
            # 尝试一个轻量级操作来检查连接
            if not self.milvus_client.has_collection(self.collection_name):
                logger.warning(f"Collection {self.collection_name} not found, recreating...")
                self.milvus_client.create_collection(
            ...
            try:
                ...
                # 重新创建客户端
                self.milvus_client = MilvusClient(self._milvus_db_path, grpc_options={"keepalive_time": _GRPC_MAX_MS, "keepalive_timeout": "20000", "keepalive_permit_without_calls": "0", "http2_min_recv_ping_interval_without_data": _GRPC_MAX_MS, "http2_min_ping_interval_without_data": _GRPC_MAX_MS})
                logger.info("Milvus client reconnected successfully")
            except Exception as reconnect_error:
                logger.error(f"Failed to reconnect Milvus: {reconnect_error}")
                raise

        # 关键修复：确保 collection 处于 loaded 状态
        try:
            self.milvus_client.load_collection(self.collection_name)
            logger.debug(f"Collection {self.collection_name} loaded successfully")
        except Exception as load_error:
            # load_collection 可能因为 collection 已经 loaded 而抛出异常，这是正常的
            logger.debug(f"Collection load status: {load_error}")
